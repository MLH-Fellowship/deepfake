...
from ml.frames_dataset import PairedDataset
from ml.logger import Logger, Visualizer
import imageio
    ...
    for it, x in tqdm(enumerate(dataloader)):
        with torch.no_grad():
            ...
            for frame_idx in range(driving_video.shape[2]):
                ...
                out = generator(
                    source_frame, kp_source=kp_source, kp_driving=kp_norm)
            ...
            imageio.imsave(os.path.join(png_dir, result_name +
                                        '.png'), (255 * predictions).astype(np.uint8))
