...
import torch
from ml.modules.util import Hourglass, AntiAliasInterpolation2d, make_coordinate_grid, kp2gaussian
...
class DenseMotionNetwork(nn.Module):
    ...
    def create_heatmap_representations(self, source_image, kp_driving, kp_source):
        ...
        zeros = torch.zeros(
            heatmap.shape[0], 1, spatial_size[0], spatial_size[1]).type(heatmap.type())
    ...
    def create_sparse_motions(self, source_image, kp_driving, kp_source):
        ...
        identity_grid = make_coordinate_grid(
            (h, w), type=kp_source['value'].type())
        identity_grid = identity_grid.view(1, 1, h, w, 2)
        coordinate_grid = identity_grid - \
            kp_driving['value'].view(bs, self.num_kp, 1, 1, 2)
        ...
        driving_to_source = coordinate_grid + \
            kp_source['value'].view(bs, self.num_kp, 1, 1, 2)
    ...
    def create_deformed_source_image(self, source_image, sparse_motions):
        ...
        """
        bs, _, h, w = source_image.shape
        source_repeat = source_image.unsqueeze(1).unsqueeze(
